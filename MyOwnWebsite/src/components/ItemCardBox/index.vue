...
function updateAnim() {
  ...
  const dyAnims = [...svg.querySelectorAll('feOffset > animate[attributeName="dy"]')] as SVGAnimateElement[]
  if (dyAnims.length >= 2) {
    dyAnims[0].setAttribute('values', `${height}; 0`)
    dyAnims[1].setAttribute('values', `0; -${height}`)
  }

  const dxAnims = [...svg.querySelectorAll('feOffset > animate[attributeName="dx"]')] as SVGAnimateElement[]
  if (dxAnims.length >= 2) {
